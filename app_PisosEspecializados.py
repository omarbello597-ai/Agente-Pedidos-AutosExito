...
if archivos:
    ...
    # =========================
    # TOTAL TIENDAS POR BDCF
    # =========================
    total_tiendas = df_bdcf.groupby('Sigla BDCF')['Cod. Tienda'].nunique().sum()
    ...
    if df_mix is not None:
        df_mix_simple = df_mix[['EAN','MIX ACTUAL','Descripción producto','MARCA']].drop_duplicates()
        df_group = df_group.merge(df_mix_simple, on='EAN', how='left')

    # =========================
    # NUEVO MIX
    # =========================
    df_group['MIX NUEVO'] = df_group['MIX ACTUAL']

    df_group.loc[df_group['decision']=="Expandir",'MIX NUEVO'] = "EXPANSIÓN"
    df_group.loc[df_group['decision']=="Reducir",'MIX NUEVO'] = "REDUCCIÓN"
