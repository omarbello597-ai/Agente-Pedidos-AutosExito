...
if archivos:
    ...
    # límites seguros
    x_min = max(df_group['rotacion'].quantile(0.05), 0.01)
    x_max = df_group['rotacion'].quantile(0.95)

    y_min = max(df_group['valor'].quantile(0.05), 1)
    y_max = df_group['valor'].quantile(0.95)

    if x_min >= x_max:
        x_min, x_max = 0.01, df_group['rotacion'].max() + 1

    if y_min >= y_max:
        y_min, y_max = 1, df_group['valor'].max() + 1
    ...
    for _, row in df_labels.iterrows():
        ...
        ax.text(row['rotacion'], row['valor'], label, fontsize=7)

    # render seguro
    try:
        st.pyplot(fig)
    except:
        st.error("Error al graficar, ajusta los filtros")
