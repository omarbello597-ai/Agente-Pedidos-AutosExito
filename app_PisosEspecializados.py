...
if archivos:
    ...
    # Validación
    if df_ventas is None or df_bdcf is None or df_mix is None:
        st.error("⚠️ Debes subir los 3 archivos: Ventas, BDCF y MIX")
        st.stop()
    ...
    x_min = max(df_group['rotacion'].quantile(0.02), 0.01)
    x_max = max(df_group['rotacion'].quantile(0.98), x_min + 0.01)

    y_min = max(df_group['valor'].quantile(0.02), 1)
    y_max = max(df_group['valor'].quantile(0.98), y_min + 1)

    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)

    for _, row in df_labels.iterrows():
        ax.text(row['rotacion'], row['valor'], str(row['EAN']), fontsize=8)
