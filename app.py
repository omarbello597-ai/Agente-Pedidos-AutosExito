...
if archivo:
    ...
    if 'estado_producto' in df_filtrado.columns:
        Estados_producto = st.sidebar.multiselect("estado_producto", sorted(df_filtrado['estado_producto'].dropna().unique()))
        if Estados_producto:
            df_filtrado = df_filtrado[df_filtrado['estado_producto'].isin(Estados_producto)]
