...
if archivo:
    ...
    if 'Tienda' in df.columns:
        tiendas = st.sidebar.multiselect("Selecciona tienda", df['Tienda'].unique())
        if tiendas:
            df = df[df['Tienda'].isin(tiendas)]

    if 'Tipo de portafolio' in df.columns:
        portafolio = st.sidebar.multiselect("Tipo de portafolio", df['Tipo de portafolio'].unique())
        if portafolio:
            df = df[df['Tipo de portafolio'].isin(portafolio)]
    ...
    col2.metric("Pedido total", int(df['pedido_ajustado'].sum()))
    ...
    with colA:
        ...
        st.dataframe(top[['pedido_ajustado','venta_promedio']].head())
    ...
    estado_counts = df['estado_producto'].value_counts()
