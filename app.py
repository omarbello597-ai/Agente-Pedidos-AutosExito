...
if archivo:
    ...
    # 🔥 LIMPIEZA
    df[ultimas_4] = df[ultimas_4].apply(pd.to_numeric, errors='coerce').fillna(0)
    ...
    # HISTÓRICO
    # =========================
    columnas_meses = [col for col in df.columns if isinstance(col, str) and "2025" in col]
    ...
    # 🔢 REDONDEO
    df['demanda_lt'] = np.ceil(df['demanda_lt'])
    df['stock_seguridad'] = np.ceil(df['stock_seguridad'])
    df['inventario_proyectado'] = np.ceil(df['inventario_proyectado'])
    ...
    # PEDIDO EN MÚLTIPLOS
    # =========================
    def ajustar_minimos(row):

        pedido = row['pedido_sugerido']
        minimo = row['Pedido Minimo a realizar']

        if minimo <= 0:
            return np.ceil(pedido)

        if pedido <= 0:
            return 0

        return np.ceil(pedido / minimo) * minimo

    df['pedido_final'] = df.apply(ajustar_minimos, axis=1)
    ...
    def tendencia(row):
        try:
            ...
            semanas = [0 if pd.isna(x) else float(x) for x in semanas]
    ...
    df['pedido_ajustado'] = np.ceil(df['pedido_ajustado'])
