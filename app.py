...
if 'Tienda' in df_filtrado.columns:
    tiendas = st.sidebar.multiselect(
        "Selecciona tienda",
        options=sorted(df_filtrado['Tienda'].dropna().unique())
    )
    if tiendas:
...
if 'Tipo de portafolio' in df_filtrado.columns:
    portafolio = st.sidebar.multiselect(
        "Tipo de portafolio",
        options=sorted(df_filtrado['Tipo de portafolio'].dropna().unique())
    )
    if portafolio:
...
if 'Segmento' in df_filtrado.columns:
    segmentos = st.sidebar.multiselect(
        "Segmento",
        options=sorted(df_filtrado['Segmento'].dropna().unique())
    )
    if segmentos:
...
if 'Formato' in df_filtrado.columns:
    formatos = st.sidebar.multiselect(
        "Formato",
        options=sorted(df_filtrado['Formato'].dropna().unique())
    )
    if formatos:
        df_filtrado = df_filtrado[df_filtrado['Formato'].isin(formatos)]

# 🔁 Reemplazar df por df_filtrado
df = df_filtrado
